...
from dpr.utils.data_utils import read_feature_data, read_data_from_json_files, Dataset
...
class JsonQADataset(Dataset):
    ...
    def _load_all_data(self):
        ...
        # filter those without positive ctx
        self.data = [r for r in data if len(r["positive_ctxs"]) > 0]
        self.feat_data = read_feature_data([r['img'] for r in self.data])
    ...
    def __getitem__(self, index) -> BiEncoderSample:
        json_sample = self.data[index]
        r = BiEncoderSample()
        r.query = self._process_query(json_sample["text"])
        ...
        def create_passage(ctx: dict):
            return BiEncoderPassage(
                normalize_passage(ctx["snippet"]) if self.normalize else ctx["snippet"],
                ctx["title"],
            )

        # set the positive, negative and hard negative cxts
        r.positive_passages = [create_passage(ctx) for ctx in positive_ctxs]
        r.negative_passages = [create_passage(ctx) for ctx in negative_ctxs]
        r.hard_negative_passages = [create_passage(ctx) for ctx in hard_negative_ctxs]

        # set the feature maps
        r.visual_embeds = self.feat_data[json_sample['img']]
